<div style="overflow-x:auto;" class="flex-container flex-column">
    <div class="flex-container" style="">
        <div>
            <p>ELASTIČNE ČIVIJE - JUS MC2.230 - DIN 1481 - din / kom</p>
            <div style="width: 700px">
                @include('components/table2', ["table" => $tables[123], "tableWidth" => 700, "col1" => "Dim.", "col1Width" => 75])
            </div>
            <div>
                <img src="/img/image425.png" alt="">
            </div>
        </div>
        <div>
            <p>BETA OSIGURAČI - DUPLI DIN 11024 - din / kom</p>
            <div class="flex-container">
                <div style="width: 150px">
                    @include('components/table1', ["table" => $tables[124], "tableWidth" => 150, "col1" => "Dim.(dxL)", "col2" => "Cena", "col1Width" => 75])
                </div>
                <div>
                    <img src="/img/image421.png" alt="">
                </div>
            </div>
        </div>
        <div>
            <p>BETA OSIGURAČI DIN 11024 - din / kom</p>
            <div class="flex-container">
                <div style="width: 150px">
                    @include('components/table1', ["table" => $tables[125], "tableWidth" => 150, "col1" => "Dim.(dxL)", "col2" => "Cena", "col1Width" => 75])
                </div>
                <div>
                    <img src="/img/image423.png" alt="">
                </div>
            </div>
        </div>
    </div>
    <div class="flex-container" style="margin-left:116px;">
        <div class="flex-container flex-column">
            <p>RASCEPKE - POCINKOVANE - JUS 300 - DIN 94 - din / kom</p>
            <div style="width: 550px">
                @include('components/table2', ["table" => $tables[126], "tableWidth" => 550, "col1" => "(dxL)", "col1Width" => 75])
            </div>
            <div>
                <img src="/img/image433.png" alt="" style="margin-bottom:235px;">
            </div>
        </div>
        <div class="flex-container flex-column">
            <div class="flex-container">
                <div>
                    <p>KARABINJER SA OPRUGOM din / 1kom</p>
                    <div class="flex-container">
                        <div style="width: 150px">
                            @include('components/table1', ["table" => $tables[127], "tableWidth" => 150, "col1" => "Dim.", "col2" => "Cena", "col1Width" => 75])
                        </div>
                        <div>
                            <img src="/img/image427.png" alt="">
                        </div>
                    </div>
                </div>
                <div>
                    <p>KARABINJER SA NAVOJEM din / 1kom</p>
                    <div class="flex-container">
                        <div style="width: 150px">
                            @include('components/table1', ["table" => $tables[128], "tableWidth" => 150, "col1" => "Dim.", "col2" => "Cena", "col1Width" => 75])
                        </div>
                        <div>
                            <img src="/img/image429.png" alt="">
                        </div>
                    </div>
                </div>
                <div>
                    <p>OSIGURAČ SA OSOVINICOM din / 1kom</p>
                    <div class="flex-container">
                        <div style="width: 150px">
                            @include('components/table1', ["table" => $tables[129], "tableWidth" => 150, "col1" => "Dim.", "col2" => "Cena", "col1Width" => 75])
                        </div>
                        <div>
                            <img src="/img/image431.png" alt="">
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex-container">
                <div class="flex-container flex-column">
                    <p>MAZALICE</p>
                    <div style="width: 350px">
                        @include('components/table2', ["table" => $tables[130], "tableWidth" => 350, "col1" => "Dim.", "col1Width" => 75])
                    </div>
                    <div class="flex-container">
                        <div>
                            <img src="/img/image441.png" alt="">
                        </div>
                        <div>
                            <img src="/img/image443.png" alt="">
                        </div>
                        <div>
                            <img src="/img/image445.png" alt="">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<div style="overflow-x:auto;" class="container">
    <div style="margin-left:70px;">
        <img src="/img/image437.png" alt="">
        <h2>Segeri</h2>
    </div>
   
</div>

<div style="overflow-x:auto;" class="flex-container">
    <div>
        <div>
            <img src="/img/image435.png" alt="">
        </div>
        <p>SEGER SPOLJNI JUS 401 - DIN 471</p>
        <div style="width: 350px">
            @include('components/table5', ["table" => $tables[131], "tableWidth" => 350, "col1" => "Dim.", "col2" => "din/kom", "col1Width" => 50, "tableNo" => 3])
        </div>
    </div>
    <div>
        <div>
            <img src="/img/image439.png" alt="">
        </div>
        <p>SEGER UNUTRAŠNJI JUS 400 - DIN 472</p>
        <div style="width: 350px">
            @include('components/table5', ["table" => $tables[132], "tableWidth" => 350, "col1" => "Dim.", "col2" => "din/kom", "col1Width" => 50, "tableNo" => 3])
        </div>
    </div>
    <div class="flex-container flex-column" style="justify-content: flex-end;">
        <div class="flex-container">
       
            <div style="width: 350px">
            <p>USKOČNIK JUS 153 - DIN 6799</p>
                @include('components/table5', ["table" => $tables[133], "tableWidth" => 350, "col1" => "Dim.", "col2" => "din/kom", "col1Width" => 75, "tableNo" => 2])
            </div>
            <div class="flex-container flex-column">
                <div>
                    <img src="/img/image449.png" alt="">
                </div>
                <div>
                    <img src="/img/image450.png" alt="">
                </div>
            </div>
        </div>
        <div>
            <p>PERFORIRANA TRAKA</p>
            <img src="/img/image451.png" alt="">
            <p>0.65/15mm x 10m</p>
        </div>
    </div>
</div>